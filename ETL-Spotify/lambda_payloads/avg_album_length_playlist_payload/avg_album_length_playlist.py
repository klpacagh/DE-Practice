...
def gather_data_local():
    ...
    with open("drum_and_bass_2021_albums.csv", 'w') as file:
...
def gather_data():
    # For every artist we're looking for
    with open("/tmp/drum_and_bass_2021_albums.csv", 'w') as file:
        header = ['Year Released', 'Album Length', 'Album Name', 'Artist']
        writer = csv.DictWriter(file, fieldnames=header)
        writer.writeheader()
        artists = get_artists_from_playlist(spotify_playlists()['rap_cavier'])
        for artist in artists.keys():
            artists_albums = spotipy_object.artist_albums(artist, album_type='album', limit=50)
            # For all of their albums
            for album in artists_albums['items']:
    ...
    date = datetime.now()
    filename = f'{date.year}/{date.month}/{date.day}/drum_and_bass_2021_albums.csv'
    response = s3_resource.Object(Bucket='spotify-analysis-data-kl', key=filename).upload_file("/tmp/drum_and_bass_2021_albums.csv")
...
if __name__ == '__main__':
    ...
    # date = datetime.now()
    # filename = f'{date.year}/{date.month}/{date.day}/drum_and_bass_2021_albums.csv'
    # s3_resource.Object('spotify-analysis-data', filename).upload_file("drum_and_bass_2021_albums.csv")